...
- `integration/`
  Stores integration tests. These tests focus on multi-module behavior, such as starting a server in-process, binding a local port, and issuing real HTTP requests.
- `common/`
...
- The preferred integration style is to start the server in-process, bind a `127.0.0.1` local port, and assert behavior through real requests.
...
- Prefer small, readable tests with one clear responsibility instead of broad tests that cover too much behavior.
