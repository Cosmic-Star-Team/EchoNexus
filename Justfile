...
configure compiler="" release="" debug_flag="":
    ...
    {{ "cmake -S . -B build/" + (if compiler != "" { replace(replace(replace(replace(replace(compiler, "/", "_"), "\\", "_"), " ", "_"), "+", "x"), ":", "_") + "-" } else { "" }) + (if release != "" { "release" } else { "debug" }) + " -G Ninja -DCMAKE_BUILD_TYPE=" + (if release != "" { "Release" } else { "Debug" }) + (if compiler != "" { " \"-DCMAKE_CXX_COMPILER=" + compiler + "\"" } else { "" }) }}
...
test label="" compiler="" export_path="" release="" debug_flag="":
    {{ if release != "" { if debug_flag != "" { error("`--debug` and `--release` are mutually exclusive") } else { "" } } else { "" } }}
    {{ "cmake -S . -B build/" + (if compiler != "" { replace(replace(replace(replace(replace(compiler, "/", "_"), "\\", "_"), " ", "_"), "+", "x"), ":", "_") + "-" } else { "" }) + (if release != "" { "release-tests" } else { "debug-tests" }) + " -G Ninja -DCMAKE_BUILD_TYPE=" + (if release != "" { "Release" } else { "Debug" }) + " -DECHONEXUS_BUILD_TESTS=ON -DECHONEXUS_BUILD_EXAMPLES=OFF" + (if compiler != "" { " \"-DCMAKE_CXX_COMPILER=" + compiler + "\"" } else { "" }) }}
